...
# species_list = ["160490-S.pyogenes"]

species_list = ["10090-M.musculus"]
...
for species in species_list:
	...
	handle_fasta = open("/home/nina/scripts/paxdb/abundance_datasets/fasta_files/%s_processed_fasta.fa" % species, "rU") #opens the fasta file containing the sequences and their information
	...
	file_write = open("/home/nina/scripts/paxdb/abundance_datasets/tables/%s_tablebis.txt" % species,'a+') 
	...
	for record in SeqIO.parse(handle_fasta, "fasta") : # for each uniprot in the fasta file : (for another obscur reason, the 2 for loops have to go in this order)
		for abundance_data in read_dataset :
			...
			if str(record.id) == str(abundance_data[1]) :# matching string id : if the string id from the mapping file and the abundance file is identical
				#print str(record.id)
