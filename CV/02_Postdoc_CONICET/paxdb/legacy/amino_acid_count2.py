...
# this version creates only one file (file_write5) with all the information : name of the protein, abundance, frequency of amino acid
# the frequencies of amino acids are given in column, in the alphabetical order
# the script doesnt take argument anymore since we have implemented a loop over all the 17 different datasets to be analyzed (species_list)
...
for species in species_list:
	...
	for record in SeqIO.parse(handle_fasta, "fasta") : # for each uniprot in the fasta file : (for another obscur reason, the 2 for loops have to go in this order)
		for mapping in read_mapping_table :
			...
			if record.id[3:9] == mapping[1] : # for a given uniprot id : if the uniprot id from the fasta file and the mapping file is identical
	...
	file_write5 = open("%s_5.txt" % species,'a+') # will print the abundance values for each protein as a column 

	handle_fasta = open("/home/nina/scripts/paxdb/abundance_datasets/fasta/%s.fasta" % species, "rU")  #for an obscur reason, handle_fasta had to be closed and reopened...
	...
	for record in SeqIO.parse(handle_fasta, "fasta"):
		...
		for i in range(0, len(abundance_list)) : # will print the abundance values for each protein as a column 
			if record.id[3:9] == abundance_list[i][0:6] : # we screen all the abundance list to match the uniprot id and retrieve the abundance value
				abundance_list[i] = abundance_list[i].split("\t")
				file_write5.write("%s" % (abundance_list[i][2]))
				#file_write5.write("%s\n" % (abundance_list[i][2])) # for some datasets we have to jump a line otherwise things get written one a single line
