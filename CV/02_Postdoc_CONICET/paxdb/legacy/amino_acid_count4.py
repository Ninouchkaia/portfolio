			...
			if record.id[3:9] == mapping[1][0:6] : # for a given uniprot id : if the uniprot id from the fasta file and the mapping file is identical
				for abundance_data in read_dataset :
					...
					if mapping[0] == abundance_data[1]: # for the same given uniprot id, corresponds a string id : if the string id from the mapping file and the abundance file is identical
						...
						file_write.write("%s\t" % dict_seq["A"]) 
						file_write.write("%s\t" % dict_seq["C"])
						file_write.write("%s\t" % dict_seq["D"])
						file_write.write("%s\t" % dict_seq["E"])
						file_write.write("%s\t" % dict_seq["F"])
						file_write.write("%s\t" % dict_seq["G"])
						file_write.write("%s\t" % dict_seq["H"])
						file_write.write("%s\t" % dict_seq["I"])
						file_write.write("%s\t" % dict_seq["K"])
						file_write.write("%s\t" % dict_seq["L"])
						file_write.write("%s\t" % dict_seq["M"])
						file_write.write("%s\t" % dict_seq["N"])
						file_write.write("%s\t" % dict_seq["P"])
						file_write.write("%s\t" % dict_seq["Q"])
						file_write.write("%s\t" % dict_seq["R"])
						file_write.write("%s\t" % dict_seq["S"])
						file_write.write("%s\t" % dict_seq["T"])
						file_write.write("%s\t" % dict_seq["V"])
						file_write.write("%s\t" % dict_seq["W"])
						file_write.write("%s\t" % dict_seq["Y"])
